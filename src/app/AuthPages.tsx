...
const inputClass =
  "w-full h-11 rounded-lg border border-slate-200 bg-white px-3.5 text-sm text-slate-900 outline-none transition placeholder:text-slate-400 focus:border-[#9CD4C1] focus:ring-4 focus:ring-[#9CD4C1]/10";
...
function AuthShell({ children }: { children: React.ReactNode }) {
  return (
    <main className="min-h-screen bg-[#f7fbf9] font-sans text-slate-900">
      <div className="grid min-h-screen lg:grid-cols-[minmax(420px,0.92fr)_minmax(500px,1.08fr)]">
        <section className="relative hidden overflow-hidden bg-[#9CD4C1] px-12 py-10 text-[#173F35] lg:flex lg:flex-col xl:px-16">
          <div className="absolute inset-0 opacity-40" style={{ backgroundImage: "radial-gradient(circle at 18% 20%, #EAF7F2 0, transparent 27%), radial-gradient(circle at 85% 78%, #8FC8B5 0, transparent 24%)" }} />
          <div className="absolute -right-24 top-28 h-80 w-80 rounded-full border border-[#4D8D78]/15" />
          <div className="absolute -right-8 top-44 h-44 w-44 rounded-full border border-[#4D8D78]/15" />
          <div className="relative z-10"><BrandLogo /></div>

          <div className="relative z-10 my-auto max-w-[520px] py-16">
            <div className="mb-6 inline-flex items-center gap-2 rounded-full border border-[#4D8D78]/20 bg-white/35 px-3 py-1.5 text-xs font-medium text-[#2F6F5C] backdrop-blur">
            ...
            <p className="mt-5 max-w-md text-[15px] leading-7 text-[#315F52]/85">
            ...
            <div className="mt-10 grid max-w-md gap-4">
              {[
                ...
                <div key={item} className="flex items-center gap-3 text-sm text-[#244F43]">
                  <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-white/45 text-[#3D806B]"><Check size={14} /></span>
                  {item}
                </div>
              ))}
            </div>
          </div>
          <p className="relative z-10 text-xs text-[#315F52]/60">Trusted tools for calm, well-stocked clinics.</p>
        </section>
...
function PrimaryButton({ children, disabled = false, onClick }: { children: React.ReactNode; disabled?: boolean; onClick?: () => void }) {
  return <button disabled={disabled} onClick={onClick} className="flex h-11 w-full items-center justify-center rounded-lg bg-[#9CD4C1] px-4 text-sm font-semibold text-[#214A3F] shadow-sm transition hover:bg-[#86BFAE] focus:outline-none focus:ring-4 focus:ring-emerald-100 disabled:cursor-not-allowed disabled:opacity-60">{children}</button>;
}
  ...
  return (
    <div className="w-full">
      ...
      <form onSubmit={submit} className="mt-7 space-y-4">
        ...
        <div>
          <div className="flex items-center justify-between"><FieldLabel>Password</FieldLabel><button type="button" onClick={() => go("forgot-password")} className="mb-1.5 text-xs font-semibold text-[#3F7665] hover:underline">Forgot password?</button></div>
          <PasswordInput value={password} onChange={setPassword} />
        </div>
        <label className="flex cursor-pointer items-center gap-2 text-xs text-slate-600"><input type="checkbox" defaultChecked className="h-4 w-4 rounded border-slate-300 accent-[#9CD4C1]" /> Keep me signed in</label>
        {error && <p className="rounded-lg bg-red-50 px-3 py-2 text-xs text-red-600">{error}</p>}
        <PrimaryButton>Sign in</PrimaryButton>
      </form>
      <p className="mt-6 text-center text-sm text-slate-500">New to HexaDent? <button onClick={() => go("sign-up")} className="font-semibold text-[#3F7665] hover:underline">Create an account</button></p>
  ...
  return (
    <div className="w-full">
      ...
      <form onSubmit={(e) => { e.preventDefault(); if (name && email.includes("@") && password.length >= 8 && agreed) setCreated(true); }} className="mt-7 space-y-4">
        ...
        <label className="flex cursor-pointer items-start gap-2.5 text-xs leading-5 text-slate-500"><input required checked={agreed} onChange={(e) => setAgreed(e.target.checked)} type="checkbox" className="mt-0.5 h-4 w-4 rounded border-slate-300 accent-[#9CD4C1]" /><span>I agree to the <span className="font-medium text-slate-700">Terms of Service</span> and <span className="font-medium text-slate-700">Privacy Policy</span>.</span></label>
        <PrimaryButton>Create account</PrimaryButton>
      </form>
      <p className="mt-6 text-center text-sm text-slate-500">Already have an account? <button onClick={() => go("sign-in")} className="font-semibold text-[#3F7665] hover:underline">Sign in</button></p>
    </div>
  );
}
...
function ForgotPassword({ go }: { go: (v: AuthView) => void }) {
  ...
  return (
    <div className="w-full">
      <button onClick={() => go("sign-in")} className="mb-7 flex items-center gap-1.5 text-xs font-semibold text-slate-500 hover:text-slate-800"><ArrowLeft size={14} /> Back to sign in</button>
      <div className="mb-5 flex h-12 w-12 items-center justify-center rounded-xl bg-[#EFF8F4] text-[#3F7665]"><Mail size={22} /></div>
...
function ResetPassword({ go }: { go: (v: AuthView) => void }) {
  ...
  return (
    <div className="w-full">
      <button onClick={() => go("sign-in")} className="mb-7 flex items-center gap-1.5 text-xs font-semibold text-slate-500 hover:text-slate-800"><ArrowLeft size={14} /> Back to sign in</button>
      <div className="mb-5 flex h-12 w-12 items-center justify-center rounded-xl bg-[#EFF8F4] text-[#3F7665]"><LockKeyhole size={22} /></div>
...
function Success({ title, body, action, onAction, secondary, onSecondary }: { title: string; body: string; action: string; onAction: () => void; secondary?: string; onSecondary?: () => void }) {
  return (
    <div className="w-full text-center">
      ...
      {secondary && <button onClick={onSecondary} className="mt-4 text-xs font-semibold text-[#3F7665] hover:underline">{secondary}</button>}
    </div>
  );
}
