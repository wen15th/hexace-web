...
const inputClass =
  "w-full h-11 rounded-lg border border-slate-200 bg-white px-3.5 text-sm text-slate-900 outline-none transition placeholder:text-slate-400 focus:border-[#315eba] focus:ring-4 focus:ring-[#315eba]/10";

function Brand() {
  return (
    <div className="flex items-center gap-2.5">
      <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-[#315eba] text-white shadow-sm shadow-blue-900/20">
  ...
  return (
    <main className="min-h-screen bg-[#f8fafc] font-sans text-slate-900">
      <div className="grid min-h-screen lg:grid-cols-[minmax(420px,0.92fr)_minmax(500px,1.08fr)]">
        <section className="relative hidden overflow-hidden bg-[#173a82] px-12 py-10 text-white lg:flex lg:flex-col xl:px-16">
          ...
          <div className="relative z-10 flex items-center gap-2.5">
            <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-white text-[#315eba]"><PackageCheck size={20} /></div>
...
function PrimaryButton({ children, disabled = false, onClick }: { children: React.ReactNode; disabled?: boolean; onClick?: () => void }) {
  return <button disabled={disabled} onClick={onClick} className="flex h-11 w-full items-center justify-center rounded-lg bg-[#315eba] px-4 text-sm font-semibold text-white shadow-sm transition hover:bg-[#274e9f] focus:outline-none focus:ring-4 focus:ring-blue-200 disabled:cursor-not-allowed disabled:opacity-60">{children}</button>;
}
  ...
  return (
    <div className="w-full">
      ...
      <form onSubmit={submit} className="mt-7 space-y-4">
        ...
        <div>
          <div className="flex items-center justify-between"><FieldLabel>Password</FieldLabel><button type="button" onClick={() => go("forgot-password")} className="mb-1.5 text-xs font-semibold text-[#315eba] hover:underline">Forgot password?</button></div>
          <PasswordInput value={password} onChange={setPassword} />
        </div>
        <label className="flex cursor-pointer items-center gap-2 text-xs text-slate-600"><input type="checkbox" defaultChecked className="h-4 w-4 rounded border-slate-300 accent-[#315eba]" /> Keep me signed in</label>
        {error && <p className="rounded-lg bg-red-50 px-3 py-2 text-xs text-red-600">{error}</p>}
        <PrimaryButton>Sign in</PrimaryButton>
      </form>
      <p className="mt-6 text-center text-sm text-slate-500">New to Hexace? <button onClick={() => go("sign-up")} className="font-semibold text-[#315eba] hover:underline">Create an account</button></p>
  ...
  return (
    <div className="w-full">
      ...
      <form onSubmit={(e) => { e.preventDefault(); if (name && email.includes("@") && password.length >= 8 && agreed) setCreated(true); }} className="mt-7 space-y-4">
        ...
        <label className="flex cursor-pointer items-start gap-2.5 text-xs leading-5 text-slate-500"><input required checked={agreed} onChange={(e) => setAgreed(e.target.checked)} type="checkbox" className="mt-0.5 h-4 w-4 rounded border-slate-300 accent-[#315eba]" /><span>I agree to the <span className="font-medium text-slate-700">Terms of Service</span> and <span className="font-medium text-slate-700">Privacy Policy</span>.</span></label>
        <PrimaryButton>Create account</PrimaryButton>
      </form>
      <p className="mt-6 text-center text-sm text-slate-500">Already have an account? <button onClick={() => go("sign-in")} className="font-semibold text-[#315eba] hover:underline">Sign in</button></p>
    </div>
  );
}
...
function ForgotPassword({ go }: { go: (v: AuthView) => void }) {
  ...
  return (
    <div className="w-full">
      <button onClick={() => go("sign-in")} className="mb-7 flex items-center gap-1.5 text-xs font-semibold text-slate-500 hover:text-slate-800"><ArrowLeft size={14} /> Back to sign in</button>
      <div className="mb-5 flex h-12 w-12 items-center justify-center rounded-xl bg-blue-50 text-[#315eba]"><Mail size={22} /></div>
...
function ResetPassword({ go }: { go: (v: AuthView) => void }) {
  ...
  return (
    <div className="w-full">
      <button onClick={() => go("sign-in")} className="mb-7 flex items-center gap-1.5 text-xs font-semibold text-slate-500 hover:text-slate-800"><ArrowLeft size={14} /> Back to sign in</button>
      <div className="mb-5 flex h-12 w-12 items-center justify-center rounded-xl bg-blue-50 text-[#315eba]"><LockKeyhole size={22} /></div>
...
function Success({ title, body, action, onAction, secondary, onSecondary }: { title: string; body: string; action: string; onAction: () => void; secondary?: string; onSecondary?: () => void }) {
  return (
    <div className="w-full text-center">
      ...
      {secondary && <button onClick={onSecondary} className="mt-4 text-xs font-semibold text-[#315eba] hover:underline">{secondary}</button>}
    </div>
  );
}
